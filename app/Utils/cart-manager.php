...
class CartManager
{
    ...
    public static function product_price_check($carts): bool
    {
        // dd($carts);
        $status = true;
        // dd($carts);
        foreach($carts as $cart){
            ...
            if ($count) {
                for ($i = 0; $i < $count; $i++) {
                    if (json_decode($product->variation)[$i]->type == $cart['variant']) {
                        if (json_decode($product->variation)[$i]->price != $cart->price) {
                            $row = Cart::find($cart->id);
                            $row->price = json_decode($product->variation)[$i]->price;
                            $row->save();
                            $status = false;
                        }
                    }
                }
            } else if (($product['product_type'] == 'physical') && $product['unit_price'] != $cart->price) {
                $row = Cart::find($cart->id);
                $row->price = json_decode($product->variation)[$i]->price;
                $row->save();
                $status = false;
