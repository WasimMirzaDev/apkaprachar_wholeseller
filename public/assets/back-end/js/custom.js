...
$('.enableSubDomain').on('change',function(){
    var val=$(this).find('option:selected').text().toUpperCase();
    if(val == 'RESELLER'){
        $("#sub_domain_div").removeClass('d-none');
    }else{
        if(!$('#sub_domain_div').hasClass('d-none')) {
            $('#sub_domain_div').addClass('d-none');
        }
    }
});
